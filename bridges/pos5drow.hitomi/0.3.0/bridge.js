...
var CURSOR_MAX_LENGTH = 4096;
var contentRatingSchema = exports_external.enum(["everyone", "mature", "adult"]);
...
var seriesInfoSchema = exports_external.object({
  ...
  contentRating: contentRatingSchema.optional(),
  shareUrl: exports_external.string().min(1).optional()
});
...
var seriesRevisionSchema = exports_external.object({
  latestChapterId: exports_external.string().min(1).optional(),
  updatedAt: exports_external.number().int().optional(),
  chapterCount: exports_external.number().int().nonnegative().optional()
}).refine((r) => r.latestChapterId !== undefined || r.updatedAt !== undefined || r.chapterCount !== undefined, { message: `a revision needs at least one field — omit the series entirely to say "don't know"` });
...
var cursorSchema = exports_external.string().min(1).max(CURSOR_MAX_LENGTH);
var pagedRequestSchema = exports_external.object({ cursor: cursorSchema.optional() });
...
// ../comical/packages/sdk/src/cursor.ts
...
function encodeCursor(value) {
  ...
  if (cursor.length > CURSOR_MAX_LENGTH) {
    throw new Error(`cursor is ${cursor.length} chars, over the ${CURSOR_MAX_LENGTH} limit — keep bulky resume state in host.storage and put its key in the cursor instead`);
  }
  return cursor;
}
function decodeCursor(cursor) {
  if (cursor === undefined)
    return;
  try {
    return JSON.parse(new TextDecoder().decode(fromBase64Url(cursor)));
  } catch {
    return;
  }
}
...
function nextPageCursor(page, hasMore) {
  return hasMore ? encodeCursor({ page: page + 1 }) : undefined;
}
// ../comical/packages/sdk/src/lang.ts
...
// ../comical/packages/sdk/src/lock.ts
class Lock {
...
var LANGUAGE_NAMES = new Set(LANGUAGES.map((l) => l.value));

// src/hitomi/query.ts
var CARD_VISIBLE_NS = new Set([
  "tag",
  "female",
  "male",
  "artist",
...
var DEFAULT_SORT = { orderby: "date", orderbykey: "added", direction: "desc" };
var DIRECTIVE = /^(?:sort|order)by(?:key|direction)?:/;
function applyDirective(token, sort) {
  ...
  if (left === "orderbydirection" || left === "sortbydirection") {
    ...
    if (d === "asc" || d === "ascending")
      sort.direction = "asc";
    else if (d === "rand" || d === "random")
      sort.direction = "random";
    else
      sort.direction = "desc";
    return;
  }
  if (right === "popular" || right === "popularity")
    sort.orderby = "popular";
  else if (right === "date")
    sort.orderby = "date";
  else if (right === "datepublished") {
    sort.orderby = "date";
    sort.orderbykey = "published";
  } else if (right === "random" || right === "rand")
    sort.direction = "random";
}
function toTerm(token) {
  const colon = token.indexOf(":");
  if (colon > 0) {
    const ns = token.slice(0, colon);
    const value = token.slice(colon + 1).trim();
    if (KNOWN_NS.has(ns) && value)
      return { ns, value };
  }
  return { ns: "", value: token.trim() };
}
function selectorTerm(raw) {
  ...
  if (ns === "tag" && sub)
    return { ns: sub[1], value: sub[2].trim() };
  if (!KNOWN_NS.has(ns))
    return;
  return { ns, value: rest.trim() };
}
function parseQuery(raw, language = "all") {
  ...
  if (selector) {
    ...
    tokens.forEach((rawToken, i) => {
      ...
      if (DIRECTIVE.test(token)) {
        if (/^(?:sort|order)(?:by)?key:/.test(token))
          orderbykeyGiven = true;
      ...
      if (token.startsWith("-"))
        negative.push(toTerm(token.slice(1)));
      else
        positive.push(toTerm(token));
    });
  }
  orGroups = orGroups.filter((g) => g.length > 0);
  if (!orderbykeyGiven && sort.orderby === "popular")
    sort.orderbykey = "year";
  let lang2 = language;
  for (let i = positive.length - 1;i >= 0; i--) {
    if (positive[i].ns === "language") {
      lang2 = positive[i].value;
      positive.splice(i, 1);
    }
  }
  positive.sort((a, b) => (a.ns ? 0 : 1) - (b.ns ? 0 : 1));
  return { positive, negative, orGroups, sort, language: lang2 };
}
function termArea(term) {
  if (!term.ns)
    return;
  if (term.ns === "female" || term.ns === "male")
    return { area: "tag", tag: `${term.ns}:${term.value}` };
  if (term.ns === "language")
    return { area: "all", tag: "index" };
  return { area: term.ns, tag: term.value };
}
function nozomiUrl(ltn, area, tag, language, sort) {
  const key = encodeURIComponent(tag);
  if (sort.orderby !== "date" || sort.orderbykey === "published") {
    if (area === "all")
      return `${ltn}/${sort.orderby}/${sort.orderbykey}-${language}.nozomi`;
    return `${ltn}/${area}/${sort.orderby}/${sort.orderbykey}/${key}-${language}.nozomi`;
  }
  if (area === "all")
    return `${ltn}/${key}-${language}.nozomi`;
  return `${ltn}/${area}/${key}-${language}.nozomi`;
}
function querySignature(q) {
  const t = (term) => `${term.ns}:${term.value}`;
  return [
...
var MAX_TERM_BYTES = 1e6;
var SCAN_CHUNK = 128;
var MAX_CARDS_PER_PAGE = 300;
var SCAN_TTL_MS = 5 * 60 * 1000;
var INDEX_VERSION_TTL_MS = 5 * 60 * 1000;
var CARD_CACHE_MAX = 600;
var FTS_NODE_SIZE = 464;
var FTS_B = 16;
var TYPES = [
...
function tagIdFromUrl(url) {
  if (!url)
    return;
  const path = decodeURIComponent(url).replace(/^\//, "");
  const lang2 = /^index-([a-z]+)(?:-\d+)?\.html$/.exec(path);
  if (lang2)
    return tagId("language", lang2[1]);
...
class HitomiBridge extends BridgeBase {
  ...
  async rangeBytes(url, start, endInclusive) {
  ...
  async nozomiSet(url, size) {
    if (size <= 0)
      return new Set;
    const { bytes } = await this.rangeBytes(url, 0, size - 1);
    return new Set(idsFromBytes(bytes));
  }
  async ftsIndexVersion() {
    if (this.ftsVersion && Date.now() - this.ftsVersion.at < INDEX_VERSION_TTL_MS)
      return this.ftsVersion.value;
    const value = (await this.fetchText(`${LTN}/galleriesindex/version`, this.headers())).trim();
    this.ftsVersion = { at: Date.now(), value };
    return value;
  }
  async ftsIds(term) {
    const version2 = await this.ftsIndexVersion();
    const indexUrl = `${LTN}/galleriesindex/galleries.${version2}.index`;
    const key = sha256(term).slice(0, 4);
    let address = 0;
    for (let depth = 0;depth <= 12; depth++) {
      const { bytes } = await this.rangeBytes(indexUrl, address, address + FTS_NODE_SIZE - 1);
      if (bytes.byteLength < FTS_NODE_SIZE)
        return [];
      const node2 = this.decodeFtsNode(bytes);
      if (!node2)
        return [];
      let i = 0;
      let cmp = -1;
      for (;i < node2.keys.length; i++) {
        cmp = compareBytes(key, node2.keys[i]);
        if (cmp <= 0)
          break;
      }
      if (cmp === 0) {
        const data2 = node2.datas[i];
        return data2 ? this.ftsData(version2, data2[0], data2[1]) : [];
      }
      const next2 = node2.subs[i];
      if (!next2)
        return [];
      address = next2;
    }
  ...
  decodeFtsNode(data2) {
    const v = new DataView(data2.buffer, data2.byteOffset, data2.byteLength);
    let pos = 0;
    const nKeys = v.getInt32(pos, false);
    pos += 4;
    if (nKeys < 0 || nKeys > FTS_B)
      return;
    const keys = [];
    for (let i = 0;i < nKeys; i++) {
      ...
      if (size <= 0 || size > 32)
        return;
      keys.push(data2.slice(pos, pos + size));
      pos += size;
    }
    const nDatas = v.getInt32(pos, false);
    pos += 4;
    if (nDatas < 0 || nDatas > FTS_B)
      return;
    const datas = [];
  ...
  async ftsData(version2, offset, length) {
    if (length <= 0)
      return [];
    const { bytes } = await this.rangeBytes(`${LTN}/galleriesindex/galleries.${version2}.data`, offset, offset + length - 1);
    if (bytes.byteLength < 4)
      return [];
    const v = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
    const count = v.getInt32(0, false);
    const ids = [];
    for (let i = 0;i < count && 4 + i * 4 + 4 <= bytes.byteLength; i++)
      ids.push(v.getInt32(4 + i * 4, false));
    return ids;
  }
  async fetchCard(id) {
    try {
      ...
      $2("a[href]").each((_i, el) => {
        const facet = tagIdFromUrl($2(el).attr("href"));
        if (facet)
          facets.add(facet);
      });
      ...
      const artists = $2(".artist-list a").map((_i, el) => $2(el).text().trim()).get().filter((s) => s.length > 0);
      if (artists.length)
        entry.subtitle = artists.join(", ");
  ...
  async card(id) {
    ...
    if (this.cardCache.size >= CARD_CACHE_MAX) {
      ...
      if (!oldest.done)
        this.cardCache.delete(oldest.value);
    }
    this.cardCache.set(id, card);
    return card;
  }
  redact(card, excluded) {
    if (excluded?.size) {
      for (const facet of card.facets) {
        if (excluded.has(facet))
          return { id: card.entry.id, title: REDACTED_TITLE, excluded: true };
      }
    }
    return card.entry;
  }
  async idsToEntries(ids, excluded) {
    const cards = await Promise.all(ids.map((id) => this.card(id)));
    return cards.filter((c) => c !== null).map((c) => this.redact(c, excluded));
  }
  excludedSet(req) {
  ...
  async plan(q) {
    ...
    for (const term of q.positive) {
      if (!term.ns)
        ftsSets.push(await this.ftsIds(term.value));
    }
    if (ftsSets.some((s) => s.length === 0)) {
      return { base: { kind: "array", ids: [] }, intersect, subtract, lateInclude, lateExclude, simple: true };
    }
    const nozomiPositives = [];
    await Promise.all(q.positive.filter((t) => t.ns).map(async (term) => {
    ...
    for (const ids of ftsSets.slice(usedFts))
      intersect.push(new Set(ids));
    for (const { term, url, size } of nozomiPositives.slice(usedNozomi)) {
      const id = tagId(term.ns, term.value);
      if (size > MAX_TERM_BYTES && CARD_VISIBLE_NS.has(term.ns))
        lateInclude.push([id]);
      else
        intersect.push(await this.nozomiSet(url, size));
    }
    ...
    for (const group of q.orGroups) {
      ...
      if (!members.length)
        continue;
      const sized = await Promise.all(members.map(async (term) => {
        if (!term.ns)
          return { term, url: undefined, size: 0 };
        const url = this.termUrl(term, q);
      ...
      for (const { term, url, size } of sized) {
        if (!url) {
          for (const id of await this.ftsIds(term.value))
            union.add(id);
        } else if (size > 0) {
          for (const id of await this.nozomiSet(url, size))
            union.add(id);
        }
      }
      intersect.push(union);
    }
    const simple = intersect.length === 0 && subtract.length === 0 && lateInclude.length === 0 && lateExclude.length === 0;
    return { base, intersect, subtract, lateInclude, lateExclude, simple };
  }
  sourceTotal(src) {
    return src.kind === "array" ? src.ids.length : src.total;
  }
  async readAt(src, start, count) {
    if (count <= 0 || start < 0)
      return [];
    if (src.kind === "array")
      return src.ids.slice(start, start + count);
    const { bytes } = await this.rangeBytes(src.url, start * 4, (start + count) * 4 - 1);
    return idsFromBytes(bytes);
  }
  async readWindow(src, offset, count, q, sig) {
  ...
  async collect(q, plan, page) {
    const sig = querySignature(q);
    const total = this.sourceTotal(plan.base);
    if (plan.simple) {
      const offset = (page - 1) * PER_PAGE;
      const ids2 = await this.readWindow(plan.base, offset, PER_PAGE, q, sig);
      const hasNext2 = q.sort.direction === "random" ? ids2.length > 0 : offset + ids2.length < total;
      return { ids: ids2, hasNext: hasNext2 };
    }
    ...
    while (scan.ids.length <= need && scan.scanned < total) {
      if (needsCards && cardsFetched >= MAX_CARDS_PER_PAGE)
        break;
      ...
      for (const set of plan.intersect)
        candidates = candidates.filter((id) => set.has(id));
      for (const set of plan.subtract)
        candidates = candidates.filter((id) => !set.has(id));
      if (!needsCards) {
      ...
      candidates.forEach((id, i) => {
        ...
        for (const clause of plan.lateInclude) {
          if (!clause.some((t) => card.facets.has(t)))
            return;
        }
        for (const t of plan.lateExclude) {
          if (card.facets.has(t))
            return;
        }
        accept(id);
      });
  ...
  async getSortOptions() {
    return SORTS.map((s) => s.state.direction === "random" ? { key: s.key, label: s.label, directionless: true } : { key: s.key, label: s.label });
  }
  applyFilters(q, req) {
    const sort = SORTS.find((s) => s.key === req.sort?.key);
    if (sort) {
      q.sort = { ...sort.state };
      if (q.sort.direction !== "random" && req.sort?.ascending)
      ...
      if (f.key === "tags") {
        ...
        for (const id of include) {
          ...
          if (!term)
            continue;
          if (term.ns === "language")
            q.language = term.value;
          else
            q.positive.push(term);
        }
        for (const id of exclude) {
          const term = termFromId(id);
          if (term)
            q.negative.push(term);
        }
        continue;
      }
      if (f.key === "type") {
        const { include, exclude } = parseFilterIncludeExclude(f.value);
        if (include.length > 1)
          q.orGroups.push(include.map((t) => ({ ns: "type", value: t })));
        else if (include[0])
          q.positive.push({ ns: "type", value: include[0] });
        for (const t of exclude)
          q.negative.push({ ns: "type", value: t });
        continue;
      }
      if ((f.key === "artist" || f.key === "series" || f.key === "character" || f.key === "group") && typeof f.value === "string") {
        for (const raw of f.value.split(",")) {
          ...
          if (entry.startsWith("-")) {
            ...
            if (value)
              q.negative.push({ ns: f.key, value });
          } else {
  ...
  async getTags(query = "") {
    ...
    const text3 = colon > 0 ? term.slice(colon + 1) : term;
    const chars = [...text3].map((c) => c === " " ? "_" : c === "/" ? "slash" : c === "." ? "dot" : encodeURIComponent(c));
    const url = `${TAGINDEX}/${field}${chars.length ? `/${chars.join("/")}` : ""}.json`;
    try {
      const rows = await this.fetchJson(url, this.headers());
  ...
  async resolveTags(ids) {
    return ids.map((raw) => raw.trim().toLowerCase()).filter((id) => id.includes(":")).map((id) => ({ id, label: tagLabel(id) }));
  }
  async fetchGallery(id) {
    if (this.lastGallery?.id === id)
      return this.lastGallery.value;
    const text3 = await this.fetchText(`${LTN}/galleries/${encodeURIComponent(id)}.js`, this.headers());
    const value = JSON.parse(text3.replace(/^var\s+galleryinfo\s*=\s*/, ""));
    this.lastGallery = { id, value };
    return value;
  }
  async getSeriesDetails(seriesId) {
    const g = await this.fetchGallery(seriesId);
    const info = { id: seriesId, title: g.title || seriesId };
    if (g.files[0]?.hash)
    ...
    if (g.language_localname || g.language)
      languages.add(g.language_localname || g.language);
    ...
    const tagGroups = [];
    const push = (label, kind, entries) => {
      if (!entries.length)
        return;
      const group = { label, tags: entries.map((e) => e.name), tagQueries: entries.map((e) => tagQuery(e.id)) };
      if (kind)
        group.kind = kind;
      tagGroups.push(group);
    };
    if (g.type)
      push("Type", "genre", [{ name: TYPE_LABELS[g.type] ?? g.type, id: tagId("type", g.type) }]);
    const themeTags = [];
    ...
    push("Series", undefined, (g.parodys ?? []).flatMap((p) => p.parody ? [{ name: p.parody, id: tagId("series", p.parody) }] : []));
    push("Characters", undefined, (g.characters ?? []).flatMap((c) => c.character ? [{ name: c.character, id: tagId("character", c.character) }] : []));
    push("Groups", undefined, (g.groups ?? []).flatMap((c) => c.group ? [{ name: c.group, id: tagId("group", c.group) }] : []));
    if (tagGroups.length)
  ...
  buildDescription(g) {
    const lines = [];
    if (g.japanese_title)
      lines.push(g.japanese_title);
    const parodys = (g.parodys ?? []).map((p) => p.parody).filter((n) => !!n && n !== "original");
    if (parodys.length)
      lines.push(`Series: ${parodys.join(", ")}`);
    const characters = (g.characters ?? []).map((c) => c.character).filter((n) => !!n);
    if (characters.length)
      lines.push(`Characters: ${characters.join(", ")}`);
    const groups = (g.groups ?? []).map((c) => c.group).filter((n) => !!n);
    if (groups.length)
      lines.push(`Circle: ${groups.join(", ")}`);
    const published = g.datepublished?.slice(0, 10);
    if (published)
      lines.push(`Published: ${published}`);
    const posted = g.date?.slice(0, 10);
    if (posted)
      lines.push(`Added: ${posted}`);
...
`) : undefined;
  ...
  async getRelatedSeries(seriesId) {
    const g = await this.fetchGallery(seriesId);
    const groups = [];
    const otherLanguages = (g.languages ?? []).map((l) => Number(l.galleryid)).filter((id) => Number.isFinite(id) && id > 0 && String(id) !== String(seriesId));
    if (otherLanguages.length) {
      const series = await this.idsToEntries(otherLanguages);
      if (series.length)
        groups.push({ label: "Other languages", kind: "alternative", series });
    }
    if (g.related?.length) {
      const series = await this.idsToEntries(g.related);
      if (series.length)
        groups.push({ label: "Related", kind: "similar", series });
    }
    return groups;
  }
...
//# debugId=9F5C57FB5FD975C564756E2164756E21
