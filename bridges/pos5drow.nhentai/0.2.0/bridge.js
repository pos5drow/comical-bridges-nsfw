...
function languageBadges(tagIds) {
  for (const id of tagIds ?? []) {
    const lang2 = LANGUAGE_TAG_IDS[id];
    if (lang2)
      return [{ text: abbreviateLanguage(lang2), position: "bottom-right", tone: "info" }];
  }
  return [];
}
...
class NhentaiBridge extends BridgeBase {
  ...
  async getListItems(listId, req = {}) {
    ...
    return {
      ...
      nextCursor: nextPageCursor(page, page < (data2.num_pages ?? 0))
    };
  }
  async getSearchResults(req) {
    ...
    for (const f of req.filters ?? []) {
      if (f.key === "language") {
        ...
        for (const lang2 of include)
          parts.push(`language:${lang2}`);
        for (const lang2 of exclude)
          parts.push(`-language:${lang2}`);
    ...
    if (!parts.length && sort === "date") {
      ...
      return {
        ...
        nextCursor: nextPageCursor(page, page < (data3.num_pages ?? 0))
      };
    }
  ...
  async getFavorites(req = {}) {
    ...
    const hasMore = data2.num_pages != null ? page < data2.num_pages : items.length >= PER_PAGE;
...
//# debugId=494A39D4888E06C164756E2164756E21
